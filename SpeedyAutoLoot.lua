...
SpeedyAutoLootDB.global.alwaysEnableAutoLoot = SpeedyAutoLootDB.global.alwaysEnableAutoLoot ~= false
...
function AutoLoot:ShowLootFrame(show)
	if self.ElvUI then
		...
		if show then
			...
			LootFrame:Show()
			if LootFrame_Update then LootFrame_Update() end
			self.isHidden = false
		else
			self.isHidden = true
			LootFrame:Hide()
...
function AutoLoot:LootItems(numItems)
	...
	for i = numItems, 1, -1 do
		...
		if GetLootSlotType then
			isMoney = GetLootSlotType(i) ~= LOOT_SLOT_ITEM
		else
			isMoney = not itemLink
		end
		local quantity, quality, locked, isQuestItem
		if self.isClassic then
			quantity, _, quality, locked, isQuestItem = select(3, GetLootSlotInfo(i))
		else
			quantity, quality, locked, isQuestItem = select(3, GetLootSlotInfo(i))
		end
		if locked or (quality and quality >= lootThreshold) then
			self.isItemLocked = true
		else
			if isMoney or (not self.isClassic and isQuestItem) or self:ProcessLoot(itemLink, quantity) then
				numItems = numItems - 1
				LootSlot(i)
			end
		end
...
function AutoLoot:PlayInventoryFullSound()
	if SpeedyAutoLootDB.global.enableSound and not self.isItemLocked then
		SafePlaySound(SpeedyAutoLootDB.global.InventoryFullSound, self.audioChannel)
	end
end
...
function AutoLoot:Help(msg)
		...
		if SpeedyAutoLootDB.global.enableSound then
			...
			if not SpeedyAutoLootDB.global.InventoryFullSound then
				...
				elseif SOUNDKIT then
					...
					SpeedyAutoLootDB.global.InventoryFullSound = "igPlayerInviteDecline"
				end
			end
	...
	elseif cmd == "set" and args ~= "" then
		...
		if SoundID then
			...
			SafePlaySound(SoundID, self.audioChannel)
...
function AutoLoot:OnLoad()
	...
	self.isClassic = (WOW_PROJECT_ID and WOW_PROJECT_ID == WOW_PROJECT_CLASSIC)
